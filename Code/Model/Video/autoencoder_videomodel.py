...
from.videomodels.autoencoder.autoencoder import EncoderAE
...
class AEVideoModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.transpose(1, 2).contiguous().view(batch * frames, chan, h, w) 

        z = self.encoder.forward(x)  

        if self.is2d:
            z = z.view(batch, frames, self.out_channels, -1)  
